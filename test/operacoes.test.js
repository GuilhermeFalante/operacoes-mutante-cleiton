...
describe('Suíte de Testes Fraca para 50 Operações Aritméticas', () => {
  ...
  test('6.1 deve lançar erro para raiz quadrada de zero e negativo', () => {
    expect(() => raizQuadrada(0)).toThrow('Não é possível calcular a raiz quadrada de um número negativo.');
  ...
  test('8.1 deve calcular corretamente fatorial de 0', () => {
    expect(fatorial(0)).toBe(1);
  });
  test('8.3 deve calcular corretamente fatorial de 1', () => {
    expect(fatorial(1)).toBe(1);
  });
  test('8.2 deve lançar erro para fatorial negativo', () => {
  ...
  test('35.1 deve retornar 1 para produto de array vazio', () => { 
    const reduceSpy = jest.spyOn(Array.prototype, 'reduce');
    expect(produtoArray([])).toBe(1); 
    expect(reduceSpy).not.toHaveBeenCalled();
    reduceSpy.mockRestore();
  });
  test('35.2 deve retornar 0 se algum elemento for zero', () => { expect(produtoArray([2, 0, 4])).toBe(0); });
  test('36. deve manter um valor dentro de um intervalo (clamp)', () => { expect(clamp(5, 0, 10)).toBe(5); });
  test('36.1 deve limitar valores fora do intervalo e manter valores na borda', () => {
    expect(clamp(-1, 0, 10)).toBe(0);
    expect(clamp(11, 0, 10)).toBe(10);
    // Importante: na borda, a implementação atual retorna o próprio valor (não passa pelos ifs)
    expect(clamp(0, 0, 10)).toBe(0);
    expect(clamp(10, 0, 10)).toBe(10);
  });
  test('36.2 deve funcionar com limites diferentes de 0', () => {
    expect(clamp(4, 5, 7)).toBe(5);
    expect(clamp(8, 5, 7)).toBe(7);
    expect(clamp(6, 5, 7)).toBe(6);
    expect(clamp(5, 5, 7)).toBe(5);
    expect(clamp(7, 5, 7)).toBe(7);
  });
  test('36.3 testar a igualdade exata na borda para matar mutante <= ou >=', () => {
    const objMin = { valueOf: () => 5 };
    const objMax = { valueOf: () => 10 };
    expect(clamp(objMin, 5, 10)).toBe(objMin);
    expect(clamp(objMax, 5, 10)).toBe(objMax);
  });
  ...
  test('38.1 deve converter ponto de ebulição (100°C)', () => { expect(celsiusParaFahrenheit(100)).toBe(212); });
  test('39. deve converter Fahrenheit para Celsius', () => { expect(fahrenheitParaCelsius(32)).toBe(0); });
  test('39.1 deve converter ponto de ebulição (212°F)', () => { expect(fahrenheitParaCelsius(212)).toBe(100); });
  ...
  test('44.1 deve retornar false quando os valores são iguais', () => { expect(isMaiorQue(5, 5)).toBe(false); });
  test('45. deve verificar se um número é menor que outro', () => { expect(isMenorQue(5, 10)).toBe(true); });
  test('45.1 deve retornar false quando os valores são iguais', () => { expect(isMenorQue(5, 5)).toBe(false); });
  test('46. deve verificar se dois números são iguais', () => { expect(isEqual(7, 7)).toBe(true); });
  test('46.1 deve retornar false para valores diferentes', () => { expect(isEqual(7, 8)).toBe(false); });
  test('47. deve calcular a mediana de um array ímpar e ordenado', () => { expect(medianaArray([1, 2, 3, 4, 5])).toBe(3); });
  test('47.1 deve calcular a mediana de um array ímpar não ordenado', () => { expect(medianaArray([3, 1, 5, 4, 2])).toBe(3); });
  test('47.2 deve calcular a mediana de um array par', () => { expect(medianaArray([1, 2, 3, 4])).toBe(2.5); });
  test('47.3 deve lançar erro para mediana de array vazio', () => {
